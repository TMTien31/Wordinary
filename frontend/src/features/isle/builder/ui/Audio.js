...
const AUDIO_BASE_URL = '/public/assets/isle/audio/';
...
export async function loadUiAudio() {
    ...
    await Promise.all([
        registerClip('ui',                `${AUDIO_BASE_URL}menu_select_lightbulb.ogg`,   { minIntervalMs: 18 }),
        // Brushing across cells fires very rapidly; allow modest overlap
        // but throttle a touch more aggressively than the UI click.
        registerClip('placement',         `${AUDIO_BASE_URL}new-placement.ogg`,            { minIntervalMs: 35 }),
        registerClip('placementWater',    `${AUDIO_BASE_URL}waterPlacement.ogg`,           { minIntervalMs: 50 }),
        registerClip('placementStone',    `${AUDIO_BASE_URL}brick-stone.ogg`,              { minIntervalMs: 35 }),
        registerClip('placementWood',     `${AUDIO_BASE_URL}fence-woodenDecorations.ogg`,  { minIntervalMs: 35 }),
        registerClip('placementVeg',      `${AUDIO_BASE_URL}small-vegetations.ogg`,        { minIntervalMs: 30 }),
        registerClip('placementTree',     `${AUDIO_BASE_URL}large-vegetations.ogg`,        { minIntervalMs: 40 }),
    ]);
}
