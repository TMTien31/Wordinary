...
export class Toolbar {
    ...
    _onClick(id) {
        ...
        switch (id) {
            ...
            case 'undo':  this.game.undo();           break;
            case 'redo':  this.game.redo();           break;
        }
    }
    ...
    update() {
        ...
        for (const [id, btn] of this.buttons) {
            ...
            btn.disabled = (id === 'undo' && !this.game.canUndo())
                || (id === 'redo' && !this.game.canRedo());
        }
    }
}
...
function drawUndoIcon(ctx) {
    ...
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.arc(1, 2, 10, Math.PI * 0.05, Math.PI * 1.55, true);
    ctx.stroke();
    ctx.fillStyle = INK;
    ctx.beginPath();
    ctx.moveTo(-10, -6);
...
function drawRedoIcon(ctx) {
    ...
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.arc(-1, 2, 10, Math.PI * 0.95, Math.PI * 1.95, false);
    ctx.stroke();
    ctx.fillStyle = INK;
    ctx.beginPath();
    ctx.moveTo(10, -6);
